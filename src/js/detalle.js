...
document.addEventListener("DOMContentLoaded", () => {
  if (!productos[id]) {
    document.body.innerHTML = "<h1 style='color:white; text-align:center; padding:50px;'>Producto no encontrado</h1>";
    return;
  }
  ...
  const detalleImgArea = document.querySelector(".detalle-img-area");
  const detalleInfoArea = document.querySelector(".detalle-info-area");
  const detalleTitle = document.getElementById("producto-nombre");
  const btnAddCart = document.getElementById("adcarrito");
  ...
  function aplicarColor(colorNombre) {
    const color = colores[colorNombre];
    if (!color) return;

    // Aplicar filtro de color a la imagen
    productoImagen.style.filter = `hue-rotate(${getHueRotate(colorNombre)}) saturate(${getSaturate(colorNombre)}) brightness(${getBrightness(colorNombre)})`;
    
    // Aplicar color a los elementos de la página
    detalleImgArea.style.border = `3px solid ${color.accent}`;
    detalleInfoArea.style.borderLeft = `4px solid ${color.primary}`;
    detalleTitle.style.color = color.primary;
    btnAddCart.style.background = `linear-gradient(120deg, ${color.primary}, ${color.secondary})`;
    
    // Aplicar color al botón de añadir al carrito
    btnAddCart.style.color = color.text;
    
    // Cambiar color de la descripción
    const descripcion = document.getElementById("producto-descripcion");
    if (descripcion) {
      descripcion.style.color = "#ccc";
    }
    
    // Cambiar color del precio según el color seleccionado
    const precio = document.getElementById("producto-precio");
    if (precio) {
      if (colorNombre === "Morado") {
        precio.style.color = "#c084fc"; // Morado claro para el precio
      } else if (colorNombre === "Azul") {
        precio.style.color = "#00ffea";
      } else {
        precio.style.color = "#00ffea";
      }
    }
    
    // Cambiar color del texto de envío
    const envio = document.querySelector(".envio");
    if (envio) {
      if (colorNombre === "Morado") {
        envio.style.color = "#a855f7"; // Morado para envío
      } else {
        envio.style.color = "#00ffaa";
      }
    }
    ...
    if (colorSelector) {
      ...
      colorSelector.style.background = "#1e1e1e";
    }
    
    // Cambiar color de las secciones inferiores
    const secciones = document.querySelectorAll(".sec-item h3");
    secciones.forEach(h3 => {
      h3.style.color = "#ffffff";
    });
    
    const secParrafos = document.querySelectorAll(".sec-item p");
    secParrafos.forEach(p => {
      p.style.color = "#ccc";
    });
    
    // Cambiar color de la descripción larga
    const descripcionSec = document.getElementById("producto-descripcion-sec");
    if (descripcionSec) {
      descripcionSec.style.color = "#ccc";
    }
    
    // Actualizar indicador visual de color
    const colorIndicator = document.getElementById("colorIndicator");
    if (colorIndicator) {
      colorIndicator.style.backgroundColor = color.primary;
      colorIndicator.style.borderColor = "#ffffff";
    }
    
    // Guardar color seleccionado
    localStorage.setItem(`color-${id}`, colorNombre);
  }

  // Función para obtener rotación de matiz según color
  function getHueRotate(colorNombre) {
    const rotaciones = {
      "Negro": "0deg",
      "Rojo": "0deg",
      "Azul": "200deg",
    ...
    return rotaciones[colorNombre] || "0deg";
  }

  // Función para obtener saturación según color
  function getSaturate(colorNombre) {
    const saturaciones = {
      "Negro": "0.5",
      "Rojo": "1.2",
      "Azul": "1.3",
    ...
    return saturaciones[colorNombre] || "1";
  }

  // Función para obtener brillo según color
  function getBrightness(colorNombre) {
    const brillos = {
      "Negro": "0.8",
      "Rojo": "1.1",
      "Azul": "1.1",
  ...
  const colorGuardado = localStorage.getItem(`color-${id}`);
  if (colorGuardado && colores[colorGuardado]) {
    colorSelector.value = colorGuardado;
    aplicarColor(colorGuardado);
  } else {
    aplicarColor("Negro"); // Color por defecto
  }

  // Botón para añadir al carrito (con color seleccionado)
  document.getElementById("adcarrito").addEventListener("click", () => {
    let carrito = JSON.parse(localStorage.getItem("carrito")) || [];
    const productoConColor = {
      ...p,
      colorSeleccionado: colorSelector.value
    };
    carrito.push(productoConColor);
    localStorage.setItem("carrito", JSON.stringify(carrito));
    alert(`🛒 Producto añadido al carrito con éxito! (Color: ${colorSelector.value})`);
  });
